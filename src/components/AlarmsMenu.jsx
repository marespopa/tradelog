...
export default function AlarmsMenu({ watchlistAlarms, onSelectSymbol }) {
  ...
  const armedCount = entries.filter(([, a]) => !a.triggered).length;
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        title="Price alarms"
        className="relative flex h-8 w-8 items-center justify-center rounded-full bg-panel-alt text-[14px] transition-all duration-200 hover:bg-panel-raised"
      >
        🔔
        {armedCount > 0 && (
                ...
                <li key={symbol} className="flex items-center justify-between gap-2 px-3 py-2 hover:bg-panel-alt">
                  ...
                  <span
                    className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium ${
                      alarm.triggered ? "bg-amber-400/15 text-amber-400" : "bg-accent/15 text-accent"
                    }`}
                  >
                    {alarm.triggered ? "Triggered" : "Armed"}
                  </span>
                  <button
                    type="button"
