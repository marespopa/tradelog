// Z-score idea applied to MORPHO 4H, same grid/discipline as
// backtest-zscore-tune.js (which ran this on ETH and found nothing that held
// up out-of-sample -- reversion combos looked great in-sample, up to 87% win
// rate, and collapsed OOS, a clean overfitting example). Re-running fresh
// for MORPHO rather than assuming the ETH result transfers: different coin,
// different volatility regime, and this repo's own MORPHO findings so far
// cut against mean-reversion working here specifically -- the RSI/Bollinger
// pullback entry in the original hand-written MORPHO strategy (a reversion-
// style trigger) failed on the full history, while pure EMA-cross
// trend-following (the opposite instinct -- chase, don't fade) is what
// finally worked (see backtest-morpho-4h-explore.js). Sweeping BOTH
// polarities anyway rather than assuming that pattern holds for z-score too.
//
// Grid-search on the first 70% of history (in-sample), only trust a
// candidate that ALSO stays positive on the held-out last 30%
...
// direction "momentum": enter long when z crosses above +entry, exit when it
// falls back to <= exit. direction "reversion": mirror image around 0 --
// enter long when z crosses below -entry (oversold), exit once it recovers
// back up to >= exit.
function simulate(candles, { zPeriod, entry, exit, direction }) {
  ...
  for (let i = WARMUP; i < candles.length; i++) {
    ...
    if (shouldEnter) position = { entryIndex: i, entryTime: bar.time, entryClose: bar.close };
  }
...
async function main() {
  ...
  console.log(`Out-of-sample (held out, never used to pick a candidate): ${new Date(splitTime).toISOString().slice(0, 10)} to ${new Date(candles.at(-1).time).toISOString().slice(0, 10)}`);
  ...
  for (const s of ranked.slice(0, TOP_N)) {
    ...
    const survives = oos.n >= MIN_IS_TRADES / 2 && oos.expectancyPct > 0 && s.inSample.compoundedPct > 0;
    ...
    console.log(
      `${direction.padEnd(10)} ${String(zPeriod).padEnd(7)} ${String(entry).padEnd(5)} ${String(exit).padEnd(4)} | ` +
        ...
        `${survives ? "YES" : oos.n < MIN_IS_TRADES / 2 ? `no (only ${oos.n} OOS trades)` : s.inSample.compoundedPct <= 0 ? "no (negative IS compounded)" : "no (negative OOS)"}`,
    );
  }
  ...
  for (const s of survivors) {
    console.log(
      `  ${s.params.direction} zPeriod=${s.params.zPeriod} entry=${s.params.entry} exit=${s.params.exit} -- ` +
        `in-sample ${s.inSample.expectancyPct >= 0 ? "+" : ""}${s.inSample.expectancyPct.toFixed(2)}%/trade (n=${s.inSample.n}, compounded ${s.inSample.compoundedPct >= 0 ? "+" : ""}${s.inSample.compoundedPct.toFixed(1)}%), ` +
        `out-of-sample ${s.oos.expectancyPct >= 0 ? "+" : ""}${s.oos.expectancyPct.toFixed(2)}%/trade (n=${s.oos.n})`,
    );
  }

  mkdirSync("backtests", { recursive: true });
  const outPath = `backtests/morpho-zscore-tune-${TIMEFRAME}-${new Date().toISOString().slice(0, 10)}.json`;
  writeFileSync(
    outPath,
    JSON.stringify(
      {
