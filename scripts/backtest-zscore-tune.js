// Parameter sweep for the z-score idea, following the same discipline as
// backtest-sol-ema200-bounce-tune.js: grid-search on the first ~70% of
// history (in-sample), then only trust a candidate that ALSO stays positive
// on the held-out last ~30% (out-of-sample, never used to pick anything). A
// combo that only wins in-sample is curve-fit noise, not an edge.
//
// Sweeps both polarities, not just the momentum reading that just lost
// (backtest-zscore-momentum-wave.js: buy z>1/sell z<=0, -32.6% compounded,
// 25% win rate on ETH 4H) -- "momentum" chases the z-score further from the
// mean, "reversion" fades it expecting a snap back (the reading
// backtest-zscore-reversion.js already found some multi-symbol signal in,
// with a different stop/target shape than tested here).
...
const MIN_IS_TRADES = 15; // below this, in-sample expectancy is too noisy to rank on
...
// falls back to <= exit (exit < entry -- a "buy strength, sell weakness"
// cycle). direction "reversion": mirror image around 0 -- enter long when z
// crosses below -entry (oversold), exit once it recovers back up to >= exit.
...
async function main() {
  ...
  const outPath = `backtests/zscore-tune-${SYMBOL.toLowerCase()}-${TIMEFRAME}-${new Date().toISOString().slice(0, 10)}.json`;
  writeFileSync(
    outPath,
    JSON.stringify(
      {
